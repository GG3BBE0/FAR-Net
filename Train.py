...
from model_improve import mymodel
...
# Training settings
parser = argparse.ArgumentParser(description='Spectroformer-implementation') # 建立一個命令列解析器
# 定義參數
...
opt = parser.parse_args() # 解析使用者從命令列輸入的參數
...
if opt.finetune is True :
    # original model training epoch
    #G_path = "checkpoint_AGSSF/uw_data/netG_model_epoch_{}.pth".format(opt.epoch_count)
    ...
    net_g = torch.load(G_path).to(device)
     ...
     if torch.cuda.device_count() > 1:
        print("使用多 GPU：", torch.cuda.device_count())
        net_g = nn.DataParallel(net_g)  # 分配到多張 GPU
